...
must_haves:
  truths:
    ...
    - "The /auth/callback, /auth/start, and /connection/status routes are excluded from JWT verification (Phase 1 has no Supabase Auth -- these are re-locked in Phase 2)"
...
Output: A working Edge Function at supabase/functions/qbo-api/index.ts with Hono routing, CORS, JWT middleware (with auth exclusions for OAuth callback, auth start, and connection status), error handler, health check route, and a QBO fetch helper that enforces minorversion=75.
</objective>
...
CRITICAL EXCLUSIONS: The following routes must be EXCLUDED from JWT verification:
...
Check `c.req.path` and skip JWT verification for paths ending in `/auth/callback`, `/auth/start`, or `/connection/status`.

Add a TODO comment on the exclusion: `// TODO(Phase 2): Remove /auth/start and /connection/status from exclusion list when Supabase Auth is wired. Only /auth/callback should remain permanently excluded.`
...
- Contains JWT verification middleware with `/auth/callback`, `/auth/start`, and `/connection/status` exclusions
- Contains TODO comment about re-locking /auth/start and /connection/status in Phase 2
...
- Run `npx supabase functions serve qbo-api --no-verify-jwt` locally to confirm it starts without syntax errors (if CLI is authenticated). If not authenticated, verify TypeScript compiles by checking for obvious syntax issues.
  ...
  <done>Edge Function scaffold is complete with Hono routing, CORS, JWT auth middleware (with /auth/callback, /auth/start, and /connection/status excluded -- TODO to re-lock in Phase 2), request logging, service client helper, qboFetch helper enforcing minorversion=75, health check, error/404 handlers, and Deno.serve bootstrap. Ready for Plan 03 to add OAuth route handlers.</done>
</task>
...
5. JWT middleware excludes `/auth/callback`, `/auth/start`, and `/connection/status` paths
6. TODO comment marks /auth/start and /connection/status for re-locking in Phase 2
7. `qboFetch` helper appends `minorversion=75` to every QBO API request
8. Error handler returns JSON (not HTML)
9. Health check route responds at GET /health
10. `Deno.serve(app.fetch)` bootstraps the server
</verification>

<success_criteria>
- Edge Function file is syntactically valid and uses correct Deno/JSR imports
- Middleware stack: CORS -> JWT auth (with exclusions for callback, start, status) -> logging
