...
must_haves:
  truths:
    ...
    - "NOTE: /connection/status and /auth/start are excluded from JWT middleware in Phase 1 (no Supabase Auth yet). Phase 2 MUST re-lock these endpoints when Supabase Auth is wired."
  artifacts:
  ...
  key_links:
    ...
    - from: "src/services/qboService.ts"
      ...
      via: "direct fetch to Edge Function URL (not supabase.functions.invoke)"
      pattern: "fetch.*functions/v1/qbo-api"
...
NOTE: /auth/start, /auth/callback, and /connection/status are all already excluded from JWT middleware by Plan 02. Do NOT modify the middleware exclusion list -- it is already correct.
...
- Confirm QBO_REDIRECT_URI secret is set (`npx supabase secrets list`) before Task 3. Without it, Intuit will return error=invalid_request on redirect.
  </verify>
 ...
 * NOTE: /connection/status is excluded from JWT middleware in Phase 1.
 * No auth header needed for this request.
 */
 ...
 * NOTE: /auth/start is excluded from JWT middleware in Phase 1.
 * No auth header needed for this request.
 */
...
NOTE: No Authorization header is sent because /auth/start and /connection/status are excluded from JWT middleware in Plan 02. This avoids the problem of sending the anon key as a Bearer token (which jose.jwtVerify would reject as non-JWT).
...
- `src/services/qboService.ts` does NOT send Authorization headers (routes are JWT-excluded)
...
- No Supabase auth tokens or QBO tokens are exposed in the frontend code
  ...
  <done>Frontend has QboConnectionStatus component in the header showing "Connect to QuickBooks" button or "Connected" badge. qboService module calls Edge Function endpoints without auth headers (routes are JWT-excluded in Phase 1). App.tsx integrates the component with hardcoded isAdmin=true (TODO for real auth later).</done>
</task>
...
- Confirm QBO_REDIRECT_URI is set: `npx supabase secrets list` -- if missing, Intuit will return error=invalid_request
...
7. Frontend does NOT send Authorization headers to JWT-excluded routes
8. URL parameters (?qbo_connected=true) handled and cleaned up
9. Connection status persists across page refresh
10. No tokens exposed in frontend code or API responses
